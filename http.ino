      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            if (page == INDEX) {
              ...
              client.println("<!DOCTYPE HTML><html><head><meta name='viewport' content='width=device-width, initial-scale=1.0'><title>ESP32SR88</title></head><body>");
              client.println("<p style='text-align:center'><b>ESP32SR88</b> <br></p>");
              ...
              client.println("<ESP32SR88DATA>");
              client.println("<RELAYS>");
              if ( digitalRead( RELAY_1 ) ) client.println("<RLY1>on</RLY1>"); else client.println("<RLY1>off</RLY1>");
              if ( digitalRead( RELAY_2 ) ) client.println("<RLY2>on</RLY2>"); else client.println("<RLY2>off</RLY2>");
              if ( digitalRead( RELAY_3 ) ) client.println("<RLY3>on</RLY3>"); else client.println("<RLY3>off</RLY3>");
              if ( digitalRead( RELAY_4 ) ) client.println("<RLY4>on</RLY4>"); else client.println("<RLY4>off</RLY4>");
              if ( digitalRead( RELAY_5 ) ) client.println("<RLY5>on</RLY5>"); else client.println("<RLY5>off</RLY5>");
              if ( digitalRead( RELAY_6 ) ) client.println("<RLY6>on</RLY6>"); else client.println("<RLY6>off</RLY6>");
              if ( digitalRead( RELAY_7 ) ) client.println("<RLY7>on</RLY7>"); else client.println("<RLY7>off</RLY7>");
              if ( digitalRead( RELAY_8 ) ) client.println("<RLY8>on</RLY8>"); else client.println("<RLY8>off</RLY8>");
              client.println("</RELAYS>");
              client.println("<INPUTS>");
              if ( !opto_states[0] ) client.println("<INP1>1</INP1>"); else client.println("<INP1>0</INP1>");
              if ( !opto_states[1] ) client.println("<INP2>1</INP2>"); else client.println("<INP2>0</INP2>");
              if ( !opto_states[2] ) client.println("<INP3>1</INP3>"); else client.println("<INP3>0</INP3>");
              if ( !opto_states[3] ) client.println("<INP4>1</INP4>"); else client.println("<INP4>0</INP4>");
              if ( !opto_states[4] ) client.println("<INP5>1</INP5>"); else client.println("<INP5>0</INP5>");
              if ( !opto_states[5] ) client.println("<INP6>1</INP6>"); else client.println("<INP6>0</INP6>");
              if ( !opto_states[6] ) client.println("<INP7>1</INP7>"); else client.println("<INP7>0</INP7>");
              if ( !opto_states[7] ) client.println("<INP8>1</INP8>"); else client.println("<INP8>0</INP8>");
              client.println("</INPUTS>");
              client.println("</ESP32SR88DATA>");
              client.println();
            }
        ...
        if (currentLine.length() == 12) {
          ...
          if ( (currentLine.substring(pos, pos + 4) == "?RLY") && (pos > 0) ) {
            ...
            switch (rly) {
              case 1:
                httpRelayAction(action, RELAY_1);
                break;
              case 2:
                httpRelayAction(action, RELAY_2);
                break;
              case 3:
                httpRelayAction(action, RELAY_3);
                break;
              case 4:
                httpRelayAction(action, RELAY_4);
                break;
              case 5:
                httpRelayAction(action, RELAY_5);
                break;
              case 6:
                httpRelayAction(action, RELAY_6);
                break;
              case 7:
                httpRelayAction(action, RELAY_7);
                break;
              case 8:
                httpRelayAction(action, RELAY_8);
                break;
            }
          }
        }
...
void httpRelayAction(int action, int relay) {
  switch (action) {
    case 0: digitalWrite(relay, 0); break;
    case 1: digitalWrite(relay, 1); break;
    case 2: digitalWrite(relay, !digitalRead(relay)); break;
  }
}
