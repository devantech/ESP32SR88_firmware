...
bool setRelay(char rly, char state)
{
  ...
  switch (rly) {
    case '1': digitalWrite(RELAY_1, state); return true;
    case '2': digitalWrite(RELAY_2, state); return true;
    case '3': digitalWrite(RELAY_3, state); return true;
    case '4': digitalWrite(RELAY_4, state); return true;
    case '5': digitalWrite(RELAY_5, state); return true;
    case '6': digitalWrite(RELAY_6, state); return true;
    case '7': digitalWrite(RELAY_7, state); return true;
    case '8': digitalWrite(RELAY_8, state); return true;
  }
  return false;
}
...
{
  switch (rly) {
    case '1': return digitalRead(RELAY_1);
    case '2': return digitalRead(RELAY_2);
    case '3': return digitalRead(RELAY_3);
    case '4': return digitalRead(RELAY_4);
    case '5': return digitalRead(RELAY_5);
    case '6': return digitalRead(RELAY_6);
    case '7': return digitalRead(RELAY_7);
    case '8': return digitalRead(RELAY_8);
  }
  return '?';
}
...
{
  switch (inp) {
    case '1': return !opto_states[0];
    case '2': return !opto_states[1];
    case '3': return !opto_states[2];
    case '4': return !opto_states[3];
    case '5': return !opto_states[4];
    case '6': return !opto_states[5];
    case '7': return !opto_states[6];
    case '8': return !opto_states[7];
  }
  return '?';
}
...
{
  int x = 0;
  if (digitalRead(RELAY_1)) x |= 0x01;
  if (digitalRead(RELAY_2)) x |= 0x02;
  if (digitalRead(RELAY_3)) x |= 0x04;
  if (digitalRead(RELAY_4)) x |= 0x08;
  if (digitalRead(RELAY_5)) x |= 0x10;
  if (digitalRead(RELAY_6)) x |= 0x20;
  if (digitalRead(RELAY_7)) x |= 0x40;
  if (digitalRead(RELAY_8)) x |= 0x80;
  return x;
}
...
{
  int x = 0;
  if (!opto_states[0]) x |= 0x01;
  if (!opto_states[1]) x |= 0x02;
  if (!opto_states[2]) x |= 0x04;
  if (!opto_states[3]) x |= 0x08;
  if (!opto_states[4]) x |= 0x10;
  if (!opto_states[5]) x |= 0x20;
  if (!opto_states[6]) x |= 0x40;
  if (!opto_states[7]) x |= 0x80;
  return x;
}
